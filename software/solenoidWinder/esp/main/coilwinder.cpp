...
int createAndBindSocket() {
    ...
    std::cout << "UDP socket created: " << sockt << ", bind..." << std::endl;
    ...
    ret = bind(sockt, (struct sockaddr*)&sock_addr, sizeof(sock_addr));
    if (ret) {
...
static void writeSocket(void *arg) {
    ...
    bcast.sin_addr.s_addr = inet_addr("172.16.1.24"); // pc
...
void wifi_station_init() {
    ...
    std::string ssid = "rj-rc-field";
    std::string pass = "r0b0jackets";
...
void mainTask(void *pvParameter)
{
  forever
  {
    ...
    if(mCommands.size())
    {
      ...
      uint32_t stepDelay2 = stepDelay * (7/cmd.wireDiameter);
